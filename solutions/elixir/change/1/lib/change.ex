defmodule Change do
  ...
  defp do_generate([head | tail], target, change)
       when head > target or has_too_many_small_coins(head, target) do
    do_generate(tail, target, change)
  end

  defp do_generate([head | tail] = coins, target, change) do
    new_target = target - head
    new_change = [head | change]

    [do_generate(coins, new_target, new_change), do_generate(tail, target, change)]
